...
def wilcoxon_signed(data1: np.ndarray, data2: np.ndarray) -> None:
...
def plot_benchmark_graph_ppi(data: pd.DataFrame, column1: str, column2: str) -> None:
    sns.regplot(
        x=column1,
        y=column2,
        data=data,
        fit_reg=False,
        marker="o",
        scatter_kws={"color": "purple", "s": 80, "alpha": 0.7},
    )
...
def plot_benchmark_graph_dekois(data: pd.DataFrame, column1: str, column2: str) -> None:
    sns.regplot(
        x=column1,
        y=column2,
        data=data,
...
def predict_rank_and_enrichment(
    model_all, model_rf, test_x, predictors, predictors_rf, scores
):
